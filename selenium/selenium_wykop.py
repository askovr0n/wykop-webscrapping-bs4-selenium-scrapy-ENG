...
# Init:
gecko_path = 'C:/Program Files (x86)/geckodriver.exe'
...
for idx_subpage in range(1, adjust_loop()+1):
    ...
    for idx_article in range(1, check_pages(SET_PAGES_TO_SCRAPE_PER_ONE_LOOP)+1):
        ...
        time.sleep(3)
        try:
            check_adv = f'/html/body/div[2]/div/div[3]/div/ul[2]/li[{idx_article}]/div[1]/div[1]/a[1]/span|/span[1]'
        ...
        try:
            if check_adv_txt != 'S':
                site_xpath = f'/html/body/div[2]/div/div[3]/div/ul[2]/li[{idx_article}]/div/div[3]/h2/a'
                ...
                print(f'Article: {idx_article} from subpage: {idx_subpage} has been successfully scraped!')
                time.sleep(3)
                driver.back()
        except:
            print(f'ARTICLE: {idx_article}, SUBPAGE {idx_subpage} HAS CONTENT FOR ADULTS OR OTHER NON-IDENTIFIED ERROR HAS OCCURED!')
